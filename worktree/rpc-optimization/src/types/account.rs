...
impl<P: Provider> Account<P> {
    ...
    pub async fn delegation_implementation(&self) -> Result<Option<Address>, RelayError> {
        // Early return if we have code override - we know it's delegated
        if self.overrides.iter().any(|(address, over)| {
            *address == self.address()
            ...
            // We know it's delegated, skip is_delegated() check
            return self.get_implementation_direct().await;
        }

        // Parallelize is_delegated and get_orchestrator since they're independent
        let (is_delegated_result, orchestrator_result) = tokio::join!(
            self.is_delegated(),
            self.get_orchestrator()
        );
        
        // Handle both potential errors explicitly
        let (is_delegated, orchestrator) = match (is_delegated_result, orchestrator_result) {
            (Ok(delegated), Ok(orch)) => (delegated, orch),
            (Err(e), Ok(_)) => {
                tracing::warn!(error = %e, "Failed to check if account is delegated");
                return Err(e.into());
            }
            (Ok(_), Err(e)) => {
                tracing::warn!(error = %e, "Failed to get orchestrator");
                return Err(e.into());
            }
            (Err(e1), Err(e2)) => {
                // Both failed - log both errors but return the first
                tracing::warn!(
                    delegation_error = %e1,
                    orchestrator_error = %e2,
                    "Both delegation check and orchestrator fetch failed"
                );
                return Err(e1.into());
            }
        };
    ...
    /// Helper method for when we know the account is delegated (e.g., when overrides are present)
    async fn get_implementation_direct(&self) -> Result<Option<Address>, RelayError> {
        let orchestrator = self.get_orchestrator().await?;
