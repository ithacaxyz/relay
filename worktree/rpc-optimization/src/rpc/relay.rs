...
impl Relay {
    ...
    pub fn new(
        ...
        let inner = RelayInner {
            ...
            // Initialize caches with 2-minute TTL and 10,000 capacity
            delegation_cache: Cache::builder()
                .time_to_live(Duration::from_secs(120))
                .max_capacity(10_000)
                .build(),
            cache_metrics: DelegationCacheMetrics::new_with_labels(&[] as &[(&str, &str)]),
        };
    ...
    /// Returns the delegation implementation address from the requested account with caching.
    ///
    ...
    async fn get_delegation_implementation<P: Provider + Clone>(
        ...
        self.get_delegation_implementation_cached(account).await
    }

    /// Returns the delegation implementation address with caching.
    #[instrument(skip_all)]
    async fn get_delegation_implementation_cached<P: Provider + Clone>(
        &self,
        account: &Account<P>,
    ) -> Result<Address, RelayError> {
        ...
        if address == Address::ZERO {
            self.inner.cache_metrics.errors.increment(1);
            tracing::warn!("Attempted to get delegation implementation for zero address");
            // Return a generic transport error for zero address
            return Err(alloy::transports::TransportErrorKind::custom_str(
                "Cannot get delegation implementation for zero address"
            ).into());
        }
        
        // Check if value is already cached
        if let Some(cached) = self.inner.delegation_cache.get(&address).await {
            self.inner.cache_metrics.hits.increment(1);
            self.inner.cache_metrics.lookup_duration.record(start.elapsed().as_nanos() as f64);
            tracing::trace!(
                account = %address,
                delegation = %cached,
                "Cache hit for delegation implementation"
            );
            return Ok(cached);
        }
        
        // Cache miss - use atomic get_with to prevent race conditions
        self.inner.cache_metrics.misses.increment(1);
        
        let account_clone = account.clone();
        let self_clone = self.clone();
        let metrics_clone = self.inner.cache_metrics.clone();
        
        let result = self.inner.delegation_cache
            .get_with(address, async move {
                let rpc_start = std::time::Instant::now();
                match self_clone.get_delegation_implementation_uncached(&account_clone).await {
                    Ok(delegation) => {
                        metrics_clone.rpc_duration.record(rpc_start.elapsed().as_nanos() as f64);
                        tracing::debug!(
                            account = %address,
                            delegation = %delegation,
                            "Cached delegation implementation (cache miss)"
                        );
                        delegation
                    }
                    Err(e) => {
                        metrics_clone.errors.increment(1);
                        tracing::warn!(
                            account = %address,
                            error = %e,
                            "Failed to get delegation implementation, returning zero address"
                        );
                        // Return zero address on error to cache the failure
                        // This prevents repeated failing RPC calls
                        Address::ZERO
                    }
                }
            })
        ...
        // Check if we got a failure placeholder
        if result == Address::ZERO {
            self.inner.cache_metrics.invalidations.increment(1);
            tracing::debug!(
                account = %address,
                "Cache returned zero address (previous failure)"
            );
            // Try to get fresh data by invalidating cache
            self.inner.delegation_cache.invalidate(&address).await;
            // Retry once
            return self.get_delegation_implementation_uncached(account).await;
        }
        ...
        tracing::trace!(
            account = %address,
            delegation = %result,
            "Retrieved delegation implementation from cache"
        );
        
        Ok(result)
    }
    
    /// Returns the delegation implementation address without caching (renamed from original method).
    #[instrument(skip_all)]
    async fn get_delegation_implementation_uncached<P: Provider + Clone>(
        &self,
        account: &Account<P>,
    ) -> Result<Address, RelayError> {
        let address = account.address();
        ...
        // Prefer on-chain result if successful
        if let Ok(Some(delegation)) = onchain_result {
            return Ok(delegation);
        }
        
        // Fall back to storage with state overrides
        let Some(stored) = storage_result? else {
            return Err(RelayError::Auth(AuthError::EoaNotDelegated(address).boxed()));
        };

        // Create account with state overrides and retry
        let account_with_overrides = account.clone().with_overrides(
        ...
        account_with_overrides.delegation_implementation().await?.ok_or_else(|| {
            RelayError::Auth(
...
pub(super) struct RelayInner {
    ...
    cache_metrics: DelegationCacheMetrics,
}
